...
{
    if (argc < 2) {
        _ERRPRINT("ZGeneralParameters::setFromArg-E-NOARG no argument given.");
        return ZS_NOTFOUND;
    }
    ...
    XmlSource = argv[1];
    if (!XmlSource.exists()) {
        _ERRPRINT("ZGeneralParameters::setFromArg-E-FILENOTEXIST given argument <%s> as general parameter file does not exist.",
                argv[1]);
        return ZS_FILENOTEXIST;
    }
    ZStatus wSt = XmlLoadAllParameters(XmlSource,GeneralParameters,DomainBroker,ErrorLog);
    delete ErrorLog;
    return wSt;
}
                             ...
                             ZaiErrors* pErrorLog)
{
    ...
    DomainBroker.setHasChanged(false);
                             ...
                             ZaiErrors *pErrorLog)
{
    ...
    if (pXmlFile==pGeneralParameters.XmlSource)
        DomainBroker.setHasChanged(false);
    return pXmlFile.writeContent(wXmlContent);
}
                             ...
                             ZaiErrors *pErrorLog)
{
    ...
    pDomainBroker.setHasChanged(false);
    return ZS_SUCCESS;
}
utf8VaryingString
