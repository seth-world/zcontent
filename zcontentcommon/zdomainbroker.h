...
public:
    ...
    bool isNull()  { return DomainRoot->childrenCount() == 0 ; }
    ...
    bool check(const utf8VaryingString& pDomainChain)
    {
        return domainChainExists(pDomainChain);
    }

    ZDomainPath* getFromDomainChain(const utf8VaryingString& pDomainChain);
    ...
    /* test if domain chain in string pDomainChain exists within defined domains and is a valid domain chain */
    bool domainChainExists(const utf8VaryingString &pDomainChain);

    ZDomainPath *_breakDomainPathName(const utf8VaryingString &pInPath, ZaiErrors *pErrorLog);

    ZDomainPathRoot* getRoot() {return DomainRoot;}

    void setHasChanged(bool pOnOff)
    {
        DomainRoot->setHasChanged(pOnOff);
    }
    bool hasChanged()
    {
        return DomainRoot->hasChanged();
    }
...
private:
    ...
    ZDomainPathRoot*        DomainRoot=nullptr;
