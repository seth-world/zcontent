...
enum ZObjectType : uint32_t {
    ...
    ZOBT_SymbolicLink =     ZDFT_SymbolicLink ,
    ZOBT_Other = ZDFT_Other  ,
    ZOBT_Hidden = ZDFT_Hidden ,
    ZOBT_All = ZDFT_All  ,
...
public:
    ...
    ZContentObjectBroker(const ZContentObjectBroker&)=delete;
    /*
    ...
    */
    ...
    /**
     * @brief getObjetFromDomain constructs then returns a ZDomainObject from pDomainChain, a valid, existing domain chain.
     */
    ZDomainObject getObjetFromDomain(const utf8VaryingString& pDomainChain, ZaiErrors* pErrorLog);

    ZDomainObject  getViewerFromType(uint32_t pType, ZaiErrors *pErrorLog);
    ZDomainObject  getEditorFromType(uint32_t pType, ZaiErrors* pErrorLog);
 ...
 */
    ...
    static bool check(const utf8VaryingString& pDomainChain);
