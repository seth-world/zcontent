...
#include "/home/gerard/Development/zbasetools/zcontent/test_zrawfile/zdocphysical_2keys.h"
#include <ztoolset/zatomicconvert.h>
#include <zindexedfile/zdataconversion.h>


 using namespace zbs;
   ZDocPhysical_2keys& ZDocPhysical_2keys::_copyFrom(const ZDocPhysical_2keys& pIn) {
...
ZDataBuffer ZDocPhysical_2keys::toRecord() {
   ZDataBuffer wReturn;
...
/*              move of individual fields to record space */
 ...
 ZStatus ZDocPhysical_2keys::fromRecord(const ZDataBuffer& pRecord) { 
   ...
   int wRank=0;
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + Documentid.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhysical_2keys::fromRecord-F-OUTBOUND Out of input boundaries while getting field <Documentid>.\n");
         return ZS_OUTBOUND;
       }
   ...
   if (FieldPresence.test(wRank++)) {
     if (wPtr + getAtomicURFSize<uint64_t>(DataRank) > wPtrEnd) {
       fprintf(stderr, "ZDocPhysical_2keys::fromRecord-F-OUTBOUND Out of input boundaries while getting field <DataRank>.\n");
       return ZS_OUTBOUND;
     }
     ...
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + Short.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhysical_2keys::fromRecord-F-OUTBOUND Out of input boundaries while getting field <Short>.\n");
         return ZS_OUTBOUND;
       }
     ...
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + Desc.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhysical_2keys::fromRecord-F-OUTBOUND Out of input boundaries while getting field <Desc>.\n");
         return ZS_OUTBOUND;
       }
   ...
   if (FieldPresence.test(wRank++)) {
     if (wPtr + getAtomicURFSize<int64_t>(DocMetaType) > wPtrEnd) {
       fprintf(stderr, "ZDocPhysical_2keys::fromRecord-F-OUTBOUND Out of input boundaries while getting field <DocMetaType>.\n");
       return ZS_OUTBOUND;
     }
   ...
   if (FieldPresence.test(wRank++)) {
     if (wPtr + getAtomicURFSize<int64_t>(Storage) > wPtrEnd) {
       fprintf(stderr, "ZDocPhysical_2keys::fromRecord-F-OUTBOUND Out of input boundaries while getting field <Storage>.\n");
       return ZS_OUTBOUND;
     }
     ...
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + MimeName.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhysical_2keys::fromRecord-F-OUTBOUND Out of input boundaries while getting field <MimeName>.\n");
         return ZS_OUTBOUND;
       }
     ...
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + URI.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhysical_2keys::fromRecord-F-OUTBOUND Out of input boundaries while getting field <URI>.\n");
         return ZS_OUTBOUND;
       }
       URI._importURF(wPtr);
     }
   if (FieldPresence.test(wRank++)) {
     if (wPtr + getAtomicURFSize<uint64_t>(DocSize) > wPtrEnd) {
       fprintf(stderr, "ZDocPhysical_2keys::fromRecord-F-OUTBOUND Out of input boundaries while getting field <DocSize>.\n");
       return ZS_OUTBOUND;
     }
     ...
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + Created.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhysical_2keys::fromRecord-F-OUTBOUND Out of input boundaries while getting field <Created>.\n");
         return ZS_OUTBOUND;
       }
     ...
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + Registrated.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhysical_2keys::fromRecord-F-OUTBOUND Out of input boundaries while getting field <Registrated>.\n");
         return ZS_OUTBOUND;
       }
     ...
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + LastModified.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhysical_2keys::fromRecord-F-OUTBOUND Out of input boundaries while getting field <LastModified>.\n");
         return ZS_OUTBOUND;
       }
     ...
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + OwnerOrigin.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhysical_2keys::fromRecord-F-OUTBOUND Out of input boundaries while getting field <OwnerOrigin>.\n");
         return ZS_OUTBOUND;
       }
   ...
   if (FieldPresence.test(wRank++)) {
     if (wPtr + getAtomicURFSize<uint32_t>(AccessRights) > wPtrEnd) {
       fprintf(stderr, "ZDocPhysical_2keys::fromRecord-F-OUTBOUND Out of input boundaries while getting field <AccessRights>.\n");
       return ZS_OUTBOUND;
     }
     ...
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + CheckSum.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhysical_2keys::fromRecord-F-OUTBOUND Out of input boundaries while getting field <CheckSum>.\n");
         return ZS_OUTBOUND;
       }
   ...
   if (FieldPresence.test(wRank++)) {
     if (wPtr + getAtomicURFSize<uint8_t>(Temporary) > wPtrEnd) {
       fprintf(stderr, "ZDocPhysical_2keys::fromRecord-F-OUTBOUND Out of input boundaries while getting field <Temporary>.\n");
       return ZS_OUTBOUND;
     }
     ...
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + Ownerid.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhysical_2keys::fromRecord-F-OUTBOUND Out of input boundaries while getting field <Ownerid>.\n");
         return ZS_OUTBOUND;
       }
     ...
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + Vaultid.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhysical_2keys::fromRecord-F-OUTBOUND Out of input boundaries while getting field <Vaultid>.\n");
         return ZS_OUTBOUND;
       }
    ...
    /* Keys */
   ZDataBuffer ZDocPhysical_2keys::getKey(long pKeyNumber) {
     switch (pKeyNumber) {
       default :
         fprintf(stderr,"ZDocPhysical_2keys::getKey-E-INVVALUE Invalid key number <%ld> while expected [0,-1].\n",pKeyNumber);
         return ZDataBuffer();
     } // switch
   } // getKey
   utf8VaryingString ZDocPhysical_2keys::getKeyName(long pKeyNumber) {
     switch (pKeyNumber) {
       default :
         fprintf(stderr,"ZDocPhysical_2keys::getKey-E-INVVALUE Invalid key number <%ld> while expected [0,-1].\n",pKeyNumber);
         return utf8VaryingString();     } // switch
   } // getKeyName
ZArray<ZDataBuffer> ZDocPhysical_2keys::getAllKeys() {
