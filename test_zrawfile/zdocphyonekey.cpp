/**  This file has been generated by zcontent utilities generator version 1.0-1
 ...
 Generation date 2023-04-02 11:57:49
...
#include "/home/gerard/Development/zbasetools/zcontent/test_zrawfile/zdocphyonekey.h"
#include <ztoolset/zatomicconvert.h>
#include <zindexedfile/zdataconversion.h>


 using namespace zbs;
   ZDocPhyOneKey& ZDocPhyOneKey::_copyFrom(const ZDocPhyOneKey& pIn) {
...
ZDataBuffer ZDocPhyOneKey::toRecord() {
   ZDataBuffer wReturn;
...
/*              move of individual fields to record space */
 ...
 ZStatus ZDocPhyOneKey::fromRecord(const ZDataBuffer& pRecord) { 
   ...
   int wRank=0;
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + Documentid.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhyOneKey::fromRecord-F-OUTBOUND Out of input boundaries while getting field <Documentid>.\n");
         return ZS_OUTBOUND;
       }
   ...
   if (FieldPresence.test(wRank++)) {
     if (wPtr + getAtomicURFSize<uint64_t>(DataRank) > wPtrEnd) {
       fprintf(stderr, "ZDocPhyOneKey::fromRecord-F-OUTBOUND Out of input boundaries while getting field <DataRank>.\n");
       return ZS_OUTBOUND;
     }
     ...
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + Short.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhyOneKey::fromRecord-F-OUTBOUND Out of input boundaries while getting field <Short>.\n");
         return ZS_OUTBOUND;
       }
     ...
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + Desc.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhyOneKey::fromRecord-F-OUTBOUND Out of input boundaries while getting field <Desc>.\n");
         return ZS_OUTBOUND;
       }
   ...
   if (FieldPresence.test(wRank++)) {
     if (wPtr + getAtomicURFSize<int64_t>(DocMetaType) > wPtrEnd) {
       fprintf(stderr, "ZDocPhyOneKey::fromRecord-F-OUTBOUND Out of input boundaries while getting field <DocMetaType>.\n");
       return ZS_OUTBOUND;
     }
   ...
   if (FieldPresence.test(wRank++)) {
     if (wPtr + getAtomicURFSize<int64_t>(Storage) > wPtrEnd) {
       fprintf(stderr, "ZDocPhyOneKey::fromRecord-F-OUTBOUND Out of input boundaries while getting field <Storage>.\n");
       return ZS_OUTBOUND;
     }
     ...
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + MimeName.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhyOneKey::fromRecord-F-OUTBOUND Out of input boundaries while getting field <MimeName>.\n");
         return ZS_OUTBOUND;
       }
     ...
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + URI.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhyOneKey::fromRecord-F-OUTBOUND Out of input boundaries while getting field <URI>.\n");
         return ZS_OUTBOUND;
       }
       URI._importURF(wPtr);
     }
   if (FieldPresence.test(wRank++)) {
     if (wPtr + getAtomicURFSize<uint64_t>(DocSize) > wPtrEnd) {
       fprintf(stderr, "ZDocPhyOneKey::fromRecord-F-OUTBOUND Out of input boundaries while getting field <DocSize>.\n");
       return ZS_OUTBOUND;
     }
     ...
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + Created.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhyOneKey::fromRecord-F-OUTBOUND Out of input boundaries while getting field <Created>.\n");
         return ZS_OUTBOUND;
       }
     ...
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + Registrated.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhyOneKey::fromRecord-F-OUTBOUND Out of input boundaries while getting field <Registrated>.\n");
         return ZS_OUTBOUND;
       }
     ...
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + LastModified.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhyOneKey::fromRecord-F-OUTBOUND Out of input boundaries while getting field <LastModified>.\n");
         return ZS_OUTBOUND;
       }
     ...
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + OwnerOrigin.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhyOneKey::fromRecord-F-OUTBOUND Out of input boundaries while getting field <OwnerOrigin>.\n");
         return ZS_OUTBOUND;
       }
   ...
   if (FieldPresence.test(wRank++)) {
     if (wPtr + getAtomicURFSize<uint32_t>(AccessRights) > wPtrEnd) {
       fprintf(stderr, "ZDocPhyOneKey::fromRecord-F-OUTBOUND Out of input boundaries while getting field <AccessRights>.\n");
       return ZS_OUTBOUND;
     }
     ...
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + CheckSum.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhyOneKey::fromRecord-F-OUTBOUND Out of input boundaries while getting field <CheckSum>.\n");
         return ZS_OUTBOUND;
       }
   ...
   if (FieldPresence.test(wRank++)) {
     if (wPtr + getAtomicURFSize<uint8_t>(Temporary) > wPtrEnd) {
       fprintf(stderr, "ZDocPhyOneKey::fromRecord-F-OUTBOUND Out of input boundaries while getting field <Temporary>.\n");
       return ZS_OUTBOUND;
     }
     ...
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + Ownerid.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhyOneKey::fromRecord-F-OUTBOUND Out of input boundaries while getting field <Ownerid>.\n");
         return ZS_OUTBOUND;
       }
     ...
     if (FieldPresence.test(wRank++)) {
       if ((wPtr + Vaultid.getURFSize()) > wPtrEnd) {
         fprintf(stderr, "ZDocPhyOneKey::fromRecord-F-OUTBOUND Out of input boundaries while getting field <Vaultid>.\n");
         return ZS_OUTBOUND;
       }
    ...
    /* Keys */
 utf8VaryingString ZDocPhyOneKey::getKey0_Name() {
   return "Primary key";
 }
 ZDataBuffer ZDocPhyOneKey::getKey0() {
   ZDataBuffer wReturn;
   /* export class using embedded URF export routine */
   Documentid._exportURF(wReturn);
   return wReturn;
 } // getKey0 
 utf8VaryingString ZDocPhyOneKey::getKey1_Name() {
   return "Secondary key test";
 }
 ZDataBuffer ZDocPhyOneKey::getKey1() {
   ZDataBuffer wReturn;
   /* export class using embedded URF export routine */
   Short._exportURF(wReturn);
   return wReturn;
 } // getKey1 
   ZDataBuffer ZDocPhyOneKey::getKey(long pKeyNumber) {
     switch (pKeyNumber) {
       case 0 :
         return getKey0();
       case 1 :
         return getKey1();
       default :
         fprintf(stderr,"ZDocPhyOneKey::getKey-E-INVVALUE Invalid key number <%ld> while expected [0,1].\n",pKeyNumber);
         return ZDataBuffer();
     } // switch
   } // getKey
   utf8VaryingString ZDocPhyOneKey::getKeyName(long pKeyNumber) {
     switch (pKeyNumber) {
       case 0 :
         return getKey0_Name();
       case 1 :
...
ZArray<ZDataBuffer> ZDocPhyOneKey::getAllKeys() {
