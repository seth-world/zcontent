...
ZStatus ZSearchParser::parse(const utf8VaryingString& pContent, ZSearchContext &pSearchContext)
{

  if (pSearchContext.isInvalid()) {
  ...
  pSearchContext.clear();

  pSearchContext.setMainPhrase(&pContent);

  ErrorLog.textLog("Processing request %s",pSearchContext.MainPhrase->toString());
...
//  ZArray<ZSearchToken*> wCurrentTokenList;
  ...
  {
      pSearchContext.CurrentTokenList.clear();
      ...
      for (; (MainIndex < ZSearchTokenizer::count()) ; MainIndex++  ) {
          if (ZSearchTokenizer::Tab(MainIndex)->Type==ZSRCH_SEMICOLON)
              break;
          pSearchContext.CurrentTokenList.push(ZSearchTokenizer::Tab(MainIndex));
      }

      if (Options & ZSRCHO_Report)
        pSearchContext.TokenReport();

       _parseContext(pSearchContext);

      if (pSearchContext.Status!=ZS_SUCCESS)
          break ;
      if (pSearchContext.CurrentTokenList.count() > 0) {
          if (pSearchContext.CurrentToken != pSearchContext.CurrentTokenList.last()) {
            int wLen = pSearchContext.CurrentTokenList.last()->TokenOffset + pSearchContext.CurrentTokenList.last()->Text.strlen()-pSearchContext.CurrentToken->TokenOffset;
            ErrorLog.warningLog("Remaining of instruction line will not be processed.\n"
                                "Ignored instructions are <%s>",
                                pSearchContext.MainPhrase->subString( pSearchContext.CurrentToken->TokenOffset, wLen ).toString());
          }
      }
  }// while
/*
...
*/
  wTi.end();
  ErrorLog.infoLog(" Parsing done. Elapsed time %s",wTi.reportElapsed().toString());

  if (pSearchContext.Store != ZSearchHistory::Forget)
    History.add(pContent);

  return pSearchContext.Status ;
}//ZSearchParser::parse
...
ZStatus
ZSearchParser::showEntity (ZSearchContext &pContext, ZSearchToken *pTokenName)
{
    if (EntityList.count()==0) {
  ...
  ErrorLog.textLog("_________________________________________Entity <%s>____________________________________________________",
                   EntityList[wi]->getEntityName().toString());
...
void
ZSearchParser::showEntityDetailsBrief (std::shared_ptr<ZSearchEntity> pEntity)
{
    ...
    ErrorLog.textLog("     Build dictionary %d fields Local dictionary (selected fields) %d fields",
                     pEntity->BuildDic.count(),pEntity->LocalMetaDic.count());

    if (pEntity->isCollection()) {
        CountTerm wCT;
        _countLogicalTerm(pEntity->LogicalTerm,wCT);

        ErrorLog.textLog("     With clause :  logical term(s):%d operands:%d operators:%d - arithmetic term(s):%d operands:%d operators:%d\n"
                         "                    end scan status: %s",
    ...
    if (pEntity->isJoin()) {
        ...
        if (pEntity->_Using != nullptr)
            ErrorLog.errorLog("     Invalid <Using> clause : not allowed for a <%s> entity",
                             pEntity->isFile()? "File":"Collection" );
    }
...
ZSearchParser::showEntityDetails (std::shared_ptr<ZSearchEntity> pEntity,int pShowType)
{
  ErrorLog.textLog("__________________Entity %s %s___________________",
    ...
    if (pShowType & 2) {
        if (pEntity->hasFormula()) {
            ErrorLog.textLog("______________________Ranks selection logical term_______________________");
            ErrorLog.textLog(pEntity->LogicalTerm->_report(0).toCChar());
         }
        else
         ErrorLog.textLog("***No record selection logical expression***");
    }
    if (pShowType & 1) {
  ...
  ErrorLog.textLog("________________end entity %s_________________",pEntity->getEntityName().toString());
...
ZSearchParser::_parseSelClause(ZSearchContext& pContext)
{
    ...
    while (pContext.notEOF() && !pContext.hasCurrent(ZSRCH_IN) )  // loop until ZSRCH_IN
    {
        ...
        if (pContext.TargetEntity->isCollection()) {
            for (int wj=0; wj < pContext.TargetEntity->_BaseEntity->LocalMetaDic.count(); wj++)
                if (pContext.CurrentToken->Text == pContext.TargetEntity->_BaseEntity->LocalMetaDic[wj].getName()) {
                    wSourceEntity = pContext.TargetEntity->_BaseEntity;
                    wRank=wj;
                    break;
            } // for
        ...
        /* Nota bene : target entity type cannot be file entity (defined by FILE clause) */

        if (pContext.hasCurrent(ZSRCH_WILDCARD)) {  /* Join is not allowed there : only Collection or File */
            pContext.TargetEntity->BuildDic.setEqualTo(wSourceEntity);            
            for (int wi = 0 ; wi < pContext.TargetEntity->LocalMetaDic.count(); wi++) {
               pContext.TargetEntity->LocalMetaDic.push(pContext.SourceEntity->LocalMetaDic[wi]);
            }
...
//     pFOD->FieldDescription = pContext.SourceEntity->LocalMetaDic[pFOD->ZSEDicRank].getFieldDescriptionPtr();
     ...
     pFOD->FullFieldName.addUtfUnit(__SEARCH_ENTITY_SEPARATOR__);
...
#ifdef __COMMENT__
ZStatus
ZSearchParser::_parseFieldIdentifier (ZSearchContext& pContext, ZArray<std::shared_ptr<ZSearchEntity>> pAllowedEntities, ZSearchField& pResultingField)
{
 ZStatus wSt=ZS_SUCCESS;
 utf8VaryingString wEntityName;
    ...
    /* if next is dot and then identifier , then previous is ZEntity name as prefix and field name follows dot  */
     ...
     if (pContext.isNestedAll()) {
         if (pContext.CurrentToken->Text != pContext.SourceEntity->getEntityName()) {
             ErrorLog.errorLog("Wrong entity identifier. Found <%s> at line %d column %d.",
                      pContext.CurrentToken->Text.toString(),
                      pContext.CurrentToken->TokenLine,pContext.CurrentToken->TokenColumn );
             pContext.LastErroredToken = pContext.CurrentToken;
    ...
    if ((pContext.CurrentTokenList.Tab(pContext.Index+1)->Type == __SEARCH_ENTITY_SEPARATOR__) && (pContext.CurrentTokenList.Tab(pContext.Index+2)->Type == ZSRCH_IDENTIFIER )) {
        /* check entity name is one of used in phrase
         * if multiple entities in pharse, scroll
         */
        if (pContext.CurrentToken->Text != pContext.SourceEntity->getEntityName()) {
        ...
        /* advance twice to point to possible field identifier */
        if (!pContext.advanceIndex())/* skip entity identifier */
            return ZS_SYNTAX_ERROR;
        if (!pContext.advanceIndex())/* skip entity separator sign to point to field identifier */
            return ZS_SYNTAX_ERROR;
    } //
    if (pContext.isNestedField()|| pContext.isNestedAll())

    if ((pContext.CurrentTokenList.Tab(pContext.Index+1)->Type == __SEARCH_ENTITY_SEPARATOR__) && (pContext.CurrentTokenList.Tab(pContext.Index+2)->Type == ZSRCH_OPERATOR_MULTIPLY )) {
        /* check entity name is one of used in phrase
         * if multiple entities in pharse, scroll
         */
        if (pContext.CurrentToken->Text != pContext.SourceEntity->getEntityName()) {
...
#ifdef __MULTIPLE_ENTITIES__
    else {
        /* if not dot, then CurEntities must have only one entity */

        if (pContext.CurEntities.count() > 1) {
            ErrorLog.errorLog("Field identifier <%s> must be prefixed with its entity name (multiple entities selected) at line %d column %d.",
                     pContext.CurrentToken->Text.toString(),
                     pContext.CurrentToken->TokenLine,pContext.CurrentToken->TokenColumn );
            return ZS_INV_ENTITY;
        }
        wCurEntitypContext.Index=0; /* get first and only current entity */




    } // dot not found
#endif //__MULTIPLE_ENTITIES__

/* just control at this convergence point */
    ...
    if (pContext.CurrentToken->Type != ZSRCH_IDENTIFIER) {
     ...
     /* here this is a valid field description : search it in current entity local dictionary (generated from field extraction using BuildDic) */

    /* if multiple entities, then modify appropriately */
#ifdef __MULTIPLE_ENTITIES__
    wCurEntitypContext.Index=0;
    for (; wCurEntitypContext.Index < pContext.CurEntities.count();wCurEntitypContext.Index++) {
        if (pContext.CurEntities[wCurEntitypContext.Index]->getEntityName() == pContext.CurrentToken->Text)
            break;
    }
    if (wCurEntitypContext.Index==pContext.CurEntities.count()) {
        ErrorLog.errorLog("Identifier<%s> is not a current entity. at line %d column %d.",
                 pContext.CurrentToken->Text.toString(),
                 pContext.CurrentToken->TokenLine,pContext.CurrentToken->TokenColumn );
     ...
     pFOD->MetaDicRank=0;
     for (;pFOD->MetaDicRank < pContext.SourceEntity->LocalMetaDic.count();pFOD->MetaDicRank++) {
         if (pContext.SourceEntity->LocalMetaDic[pFOD->MetaDicRank].getName()==pContext.CurrentToken->Text)
         break;
     }

     if (pFOD->MetaDicRank==pContext.SourceEntity->LocalMetaDic.count()) {
       ErrorLog.errorLog("Wrong field name <%s> for single entity <%s> (field not found in entity dictionary) at line %d column %d.",
           pContext.CurrentTokenList.Tab(pContext.Index+2)->Text.toString(),
    ...
    pFOD->MetaDicPtr = &pContext.SourceEntity->LocalMetaDic; /* store a pointer to Local MetaDic object */

    ZSearchOperandType_type wZSTO = getZSTOFromZType(pContext.SourceEntity->LocalMetaDic[pFOD->MetaDicRank].ZType);
    pFOD->setZSTO(wZSTO | ZSTO_Field);

//     pFOD->FieldDescription = pContext.SourceEntity->LocalMetaDic[pFOD->ZSEDicRank].getFieldDescriptionPtr();
     pFOD->FullFieldName = pContext.SourceEntity->EntityFullName;
     pFOD->FullFieldName.addUtfUnit('.');
     pFOD->FullFieldName += pContext.SourceEntity->LocalMetaDic[pFOD->MetaDicRank].getName();

     _DBGPRINT("ZSearchParser::_parseFieldIdentifier Found field operand <%s>\n"
               "        ZType <%s> ZSTO Type<%s>  Dictionary rank %d \n",
               pFOD->FullFieldName.toString(),
               decode_ZType(pFOD->getZType()),
               decode_OperandType(pFOD->ZSTO), pFOD->MetaDicRank
               )

//     pFOD->TokenList.push(pContext.CurrentToken); /* store field identifier token */

     if (!pContext.advanceIndex())/* skip field identifier */
       return ZS_SYNTAX_ERROR;

     /* check if modifier */
     if ((pContext.CurrentToken->Type & ZSRCH_MODIFIER)!=ZSRCH_MODIFIER)
         return ZS_SUCCESS;  /* no modifier */

     return _parseModifier(pFOD,pContext);
                                               ...
                                               int pParenthesisLevel)
{
  ...
  while (true) {
    ...
    if (pContext.CurrentToken->Type == ZSRCH_OPENPARENTHESIS) {
      ...
      pParenthesisLevel++;
      wSt=_parseLogicalTerm((ZSearchLogicalTerm* &)pOperand,pContext,pParenthesisLevel+1,pContext.Index);
      if (wSt!=ZS_SUCCESS) {
        return wSt;
      }
...
//  ZSearchOperandType_type wOpType = ZSTO_Nothing;
  ...
  if (!ZSearchOperator::isComparator(pContext.CurrentToken)) {
    ErrorLog.errorLog("Invalid logical operator <%s> at line %d column %d.",
        pContext.CurrentToken->Text.toString(),
...
//  wSvIndex = pContext.Index;
  ...
  if (ZSearchOperator::isAndOr(pContext.CurrentToken)) {
    ...
    wSt=_parseLogicalTerm(wTerm->NextTerm,pContext,pParenthesisLevel,pContext.Index);
    if (wSt!=ZS_SUCCESS) {
      delete wTerm;
      return wSt;
    }
                                            ...
                                            bool &pIsLiteral)
{
  ...
  if (!_arithmeticTypeCheck(pRequestedType,pContext,wOB,wSvIndex)) {
      delete wArithTerm;
      return pContext.Status=ZS_INVTYPE;
  }
  ...
  wSt=_parseArithmeticTerm((ZSearchArithmeticTerm*&)wArithTerm->OperandNext,pContext,pParenthesisLevel,pContext.Index,pRequestedType,pIsLiteral);
  if (wSt!=ZS_SUCCESS) {
    delete wArithTerm;
    return wSt;
  }
...
//          if (wSt!=ZS_SUCCESS) {
  ...
  if (wi == MasterFileList.count())  {  /* not found in master file list : create one in list */
    wMF=std::make_shared<ZMasterFile>();
//    wMF->_setErrorLog.errorLog(getErrorLog.errorLog());  /* assign parser errorlog to masterfile */
    if (pContext.InstructionType & ZSITP_Modify) {
      wSt=wMF->zopen(wPath,ZRF_Modify);
    }
    else {
      wSt=wMF->zopen(wPath,ZRF_Read_Only);
    }
    if (wSt!=ZS_SUCCESS) {
...
//  std::shared_ptr<ZSearchEntity> wEntityPtr= ZSearchEntity::constructWithMaster(wMF,wEntityToken);

  pContext.TargetEntity = std::make_shared<ZSearchEntity>(wMF,pContext.TokenIdentifier->Text);
...
ZSearchParser::_parseContextDeclareEntity(ZSearchContext & pContext)
{
  ...
  if (pContext.isEOF()||(pContext.CurrentToken->Type != ZSRCH_JOIN)) {
...
/* target entity is instantiated as soon as source entity name is known and validated to be a valid, registered entity */
      ...
      pContext.TargetEntity = std::shared_ptr<ZSearchEntity>(new ZSearchEntity(pContext.SourceEntity,pContext.TokenIdentifier));
      ...
      if (pContext.notEOF() &&
          ...
          ErrorLog.errorLog("Expected one of { <WITH> , <USING> , <JOIN>. Found <%s> at line %d column %d.",
                   ...
                   pContext.CurrentToken->TokenLine,pContext.CurrentToken->TokenColumn  );
          pContext.LastErroredToken=pContext.CurrentToken;
//          delete pContext.TargetEntity;
          return pContext.Status=ZS_SYNTAX_ERROR;
      }
 #ifdef __COMMENT__
      while (pContext.notEOF() && (pContext.CurrentToken->Type != ZSRCH_WITH) && (pContext.CurrentToken->Type != ZSRCH_JOIN))  {

          /* search for a valid open entity as source for current entity */
          std::shared_ptr<ZSearchEntity>  wEntity = EntityList.getEntityByName(pContext.CurrentToken->Text);
          if (wEntity==nullptr) {
              ErrorLog.errorLog("Entity identifier <%s> has not been found in active entities list. at line %d column %d.",
                pContext.CurrentToken->Text.toString(),
                pContext.CurrentToken->TokenLine,pContext.CurrentToken->TokenColumn  );
  ...
  /*entity is instantiated as soon as first entity name is known and validated to be a valid, stored entity */

          pContext.SourceEntity = wEntity ;
          pContext.TargetEntity = std::shared_ptr<ZSearchEntity>(new ZSearchEntity(wEntity,pContext.TokenIdentifier));
...
//  ZStatus wSt=ZS_SUCCESS;
  ...
  pContext.Status =_parseLogicalTerm(pContext.TargetEntity->_Using,pContext,0,0);
...
{
    if (!pContext.InstructionType & ZSITP_ToBeExecuted)
        return ZS_SUCCESS;
...
ZSearchParser::executeFetch(ZSearchContext & pContext)
{
    ...
    if (pContext.InstructionType & ZSITP_All) {
        return pContext.SourceEntity->populateAll();
    }
    return wSt;
}
/*
...
ZSearchParser::_parseContextShow(ZSearchContext& pContext)
{
  ZStatus wSt=ZS_SUCCESS;
    ...
    case ZSRCH_ENTITY:
    {
        ...
        showEntity (pContext,pContext.CurrentToken);
        pContext.advanceIndex(false);
        return pContext.Status ;
...
//        pContext.advanceIndex(false);
    ...
    {
        return pContext.Status = _parseContextShowEntity(pContext);
    }//ZSRCH_IDENTIFIER
    default:
    {
...
ZSearchParser::loadXmlSearchParserZEntity(const uriString& pXmlFile)
{
  ...
  while (true) {
    ...
    ErrorLog.textLog("_________________Search Parser ZEntity table load ____________________\n"
                              " %ld ZEntity values loaded.\n", ZEntitySymbolList.count());
    ErrorLog.textLog("%3s- %27s %10s\n","rnk","ZEntity","Value");
    for (long wi=0;wi<ZEntitySymbolList.count();wi++) {
      ErrorLog.textLog("%3ld- <%25s> <0x%08lX>\n",wi+1,
...
ZStatus DisplayEntityDefault(std::shared_ptr<ZSearchEntity> pEntity)
{
    return ZS_SUCCESS;
}
