...
public:
  ...
  friend class ZObjectSelector;

  explicit ZDomainDLg(QWidget* pParent) ;
  ~ZDomainDLg() override ;

  void initLayout();
  void setupCreate(ZDomainPath* pFatherDomainPath) ;
  void setupChange(ZDomainPath* pDomainPath);

  void setContent(const utf8VaryingString& pContent);
  void setDomainName(const utf8VaryingString& pName);
  void setObjectType(ZDomain_type pType);

  ZDomainPath* getDomainPathRef() {return DomainPathRef;}
