...
ZDomainDLg::initLayout()
{
  ...
  QLabel* wFDN=new QLabel("Father domain name");
  ...
  QForm->addRow(new QLabel("Domain name",this),NameLEd);
...
ZDomainDLg::setupChange(ZDomainPath* pDomainPath, ZDomainPath *pDomainRoot)
{
    ...
    RelativeRBn->setChecked(DomainPathRef->Type & ZDOM_Relative);
    AbsoluteRBn->setChecked(!(DomainPathRef->Type & ZDOM_Relative));
...
*/
    ...
    if (DomainPathRef->Name==DomainPathRef->Father->Name) {
        ZExceptionDLg::adhocMessage("Domain name",Severity_Error,
                                    "Domain name <%s> cannot be the same of its superior.",DomainPathRef->Name.toString());
        NameLEd->setFocus();
        return;
    }
