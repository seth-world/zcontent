 ...
 */
ZStatus
ZSSearchArgument::add(const ssize_t pFieldOffset,
                             const ssize_t pFieldLength,
                             ZDataBuffer &pSequence,
                             const uint16_t pSearchType,
                             const bool pOR)
{
     newBlankElement();  // no use of pClear : can memset this
     last().OR = pOR;
 ...
 */
bool
ZSSearchArgument::evaluate (ZDataBuffer &pRecord)
{
 ...
 */
void
ZSSearchArgument::report(FILE*pOutput)
{
    fprintf(pOutput,
            "----------Search arguments-----------\n"
