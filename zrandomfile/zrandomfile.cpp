...
ZStatus
ZOpenZRFPool::removeFileByObject(ZFileDescriptor *pZRF)
{
ZStatus wSt=ZS_NOTFOUND;
    ...
    for (long wi=0;wi<size();wi++)
    {
        ...
        {
        if (Tab[wi]->_isOpen)
          fprintf(stderr,"ZOpenZRFPool::removeFileByObject-W-ISOPEN file <%s> is open while removing its descriptor from open pool.", Tab[wi]->getURIContent().toCChar());
        erase(wi,1);
        return ZS_SUCCESS;
        }
    }// for
...
//    if (__ZRFVERBOSE__)
    ...
    while (size())
    {
        ...
        if (wZRF->_isOpen)
            ...
            wSt=wZRF->_close();
            if (wSt!=ZS_SUCCESS)
              fprintf(stderr,"ZOpenZRFPool::closeAll-E-ERRCLOSE error <%s> while closing file <%s>.",decode_ZStatus(wSt), wZRF->getURIContent().toCChar());
            wNB++;
            }// wZRF->isOpen()
...
int wRet;
  ...
  HeaderFd = open(URIHeader.toCString_Strait(),O_WRONLY|O_CREAT|O_TRUNC,wPosixMode);
  if (HeaderFd<0)
            {
            ZException.getErrno(errno,
                            _GET_FUNCTION_NAME_,
...
ZStatus
ZRandomFile::_readFileLock( lockPack_struct &pLockPack)
{
    ...
    ssize_t wSRead =read(HeaderFd,(char *)&pLockPack,sizeof(lockPack_struct));  // read lock infos with packed structure
...
ZStatus
ZRandomFile::_writeFileLock(lockPack_struct &pLockPack)
{
    ...
    ssize_t wSWrite =write(HeaderFd,(char *)&wLockPack,sizeof(lockPack_struct)); // write lock infos to file header s
     if (wSWrite<0)
             {
             ZException.getErrno(errno,
                                 _GET_FUNCTION_NAME_,
...
//    ssize_t wSRead =read(HeaderFd,wZDB.DataChar,wZDB.Size);  // load the whole file in wZDB
  ...
  wSt=ZHeader._import((unsigned char *)&wZHeaderExport);
...
ZDataBuffer wHeaderContent;
    ...
    if ((wSt=ZHeader._import((unsigned char *)wPtr))!=ZS_SUCCESS)
            {return   wSt;}
...
ZRandomFile::_getFileControlBlock ( bool pForceRead)
{
  if (getOpenMode() & ZRF_Exclusive)
    ...
    ZPMS.HFDReads ++;
    ssize_t wSRead =read(HeaderFd,wZDB.DataChar,sizeof(ZFileControlBlock_Export));  //! read at first Header control block
    if (wSRead<0)
        {
        ZException.getErrno(errno,
                         _GET_FUNCTION_NAME_,
...
{
    return  _open(pMode,ZFT_ZRandomFile);
 ...
 * @param[in] pFileType      a ZFile_type defining the type of file to open. ZFT_Any allows to open any file.
 *
...
void
ZRandomFile::zsurfaceScan(const uriString pURIContent, FILE *pOutput)
{
ZStatus wSt;
 ...
 */
void
ZRandomFile::_fullcontentDump(const int pColumn, FILE* pOutput)
 {
ZStatus wSt;
